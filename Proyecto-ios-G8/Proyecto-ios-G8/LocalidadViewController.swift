...
class LocalidadViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {

    
    @IBOutlet var localidadLbl: UILabel!
    @IBOutlet weak var provinciaText: UITextField!
    var select: String?
    var lista = ["San José","Alajuela","Cartago","Heredia","Guanacaste","Puntarenas","Limon"]
    var count = 0
    var count2 = 0
    ...
    override func viewDidLoad() {
        ...
        self.localidadLbl.text = localidad
        getSitios(index:0)
    ...
    func getSitios(index: Int){
        let url = URL(string: "http://apiexpertos-001-site1.itempurl.com/api/Tarea/Localidad?Provincia="+self.localidad)!
    ...
    @IBAction func buscar(_ sender: Any){
        if menuOption == true {
            ...
            self.localidad = self.provinciaText.text
            self.localidadLbl.text = self.localidad
            hideMenu()
            menuOption = false
            menuOption = false
            menuHide.isEnabled = false
...
extension LocalidadViewController: UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate{
