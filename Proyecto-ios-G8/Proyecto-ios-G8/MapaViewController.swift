...
//  Proyecto-ios-G8
//
//  Created by LABORATORIOS on 15/6/22.
//

import UIKit

class MapaViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func inicio(){
        let vc = storyboard?.instantiateViewController(identifier: "inicio_vc") as! ViewController
        vc.modalPresentationStyle = .fullScreen
        present(vc, animated: true)
    }
    
    @IBAction func sitios(){
        let vc = storyboard?.instantiateViewController(identifier: "sitio_vc") as! SitiosTuristicosViewController
        vc.modalPresentationStyle = .fullScreen
        present(vc, animated: true)
    }
    
    @IBAction func mapa(){
        let vc = storyboard?.instantiateViewController(identifier: "mapa_vc") as! MapaViewController
        vc.modalPresentationStyle = .fullScreen
        present(vc, animated: true)
    }
    
    @IBAction func oferta(){
        let vc = storyboard?.instantiateViewController(identifier: "oferta_vc") as! OfertasViewController
        vc.modalPresentationStyle = .fullScreen
        present(vc, animated: true)
    }
    
    @IBAction func galeria(){
        let vc = storyboard?.instantiateViewController(identifier: "galeria_vc") as! GaleriaViewController
        vc.modalPresentationStyle = .fullScreen
        present(vc, animated: true)
    }

    @IBAction func credito(){
        let vc = storyboard?.instantiateViewController(identifier: "contacto_vc") as! CreditoViewController
        vc.modalPresentationStyle = .fullScreen
        present(vc, animated: true)
    }
}

extension MapaViewController: UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate{
