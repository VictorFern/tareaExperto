...
class SitiosTuristicosViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func verRuta(_ sender: Any){
        let subtitulo = "\n Sobre la ruta" as NSString
        let descripcion = "\n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Magnam inventore deleniti aut dolorum nobis autem rerum commodi, consequuntur eaque, velit id nihil ipsam quam dolor iusto exercitationem quas veniam sed. Lorem ipsum, dolor sit amet consectetur adipisicing elit. Possimus doloribus mollitia facilis maiores qui reprehenderit tempore velit eligendi quasi enim, labore similique quod facere ipsa ut sed quaerat exercitationem fugit." as NSString
        let msg = (subtitulo as String) + (descripcion as String)
        let alert = UIAlertController(title: "Ruta", message: msg, preferredStyle: .alert)
        let imageView = UIImageView(frame: CGRect(x: 10, y: 300, width: 250, height: 230))
        imageView.image = UIImage(named: "museo-del-jade-costa-rica")
        let height = NSLayoutConstraint(item: alert.view!, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: 580)
